...
import { useTeacherStoreGroup } from "../../stores/teacher/group";
...
const store = useTeacherStoreGroup();
...
// ID.value = profile?.profile?.id;
// watch()

onMounted(async () => {
  ID.value = await profile.getProfile();
  count.value = await store.getGroupById(ID.value.id);
